...
#' @param consolidate_ambiguous_alignments consolidate alleles that represent equally valid perfect alignments into a single representative allele
...
#' }
#' @export
summarize_candidates = function( reads, transcript, pathogenic_mutation, consolidate_ambiguous_alignments=TRUE ){
    pos = c()
    chroms = c()
    transcript_id = c()
    evidence = c()
    ...
    if( consolidate_ambiguous_alignments ){
        df_sum = consolidate_ambiguous_candidates(df_sum, transcript)
    }
    list( summary = df_sum,
...
#' Consolidate duplicate candidate reversions
#'
#' @param df_sum Read summary generated by aardvark
#' @param transcript aardvark::TranscriptData object for a gene's transcript
#'
#'
consolidate_ambiguous_candidates = function( df_sum, transcript ){
    # Identify a specific, recurent form of duplication: two deletions that are
    # only distinguished by a difference in where the alignment placed a few nucleotides that are
    # present on both ends of the deletion, e.g.
    # REF TATTACCGGATAGC
    # #1  TA--------TAG
    # #2  TAT--------AGC
